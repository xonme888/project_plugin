...
2. 같은 의미를 다른 이름이나 error code로 표현하는 중복을 찾는다.
3. Backend 구현 세부 패키지나 Frontend 컴포넌트 구조가 아니라 wire contract 기준으로 정리한다.
4. 변경이 필요한 경우 `api-contract-change`로 additive, behavioral, breaking 여부를 분류한다.
5. 확정 전에는 `[계약 질문]` 또는 `[PM 결정 요청]` 초안만 작성한다.
6. 확정된 규칙만 product API 명세와 관련 문서에 반영한다.
